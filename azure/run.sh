...
cat values-public-dns.yaml | envsubst | helm install external-dns ~/code/bitnami-charts/bitnami/external-dns \
...
kubectl --namespace external-dns create secret generic azure-external-dns-secret \
    --from-file=azure.json --dry-run -o yaml | kubectl apply -f -

helm install external-dns ~/code/bitnami-charts/bitnami/external-dns \
    --namespace external-dns \
    --set publishInternalServices=true \
    --set logLevel=debug \
    --set interval=15s \
    --values values-public-dns-secretname.yaml
